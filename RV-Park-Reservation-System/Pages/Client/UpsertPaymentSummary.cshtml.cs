...
{
    public class UpsertPaymentSummaryModel : PageModel
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly UserManager<ApplicationCore.Models.Customer> _userManager;
        private readonly IEmailSender _emailSender;

        public UpsertPaymentSummaryModel(IUnitOfWork unitOfWork, UserManager<ApplicationCore.Models.Customer> userManager, IEmailSender emailSender)
        {
        ...
        public IActionResult OnGet(bool? error, string returnUrl = null)
        {
            ...
            if (!User.Identity.IsAuthenticated && !User.IsInRole("Customer")) //Check if the user is authenticated and is in customer role
            {
                return RedirectToPage("/Shared/Prohibited", new { path = "/Client/UpsertPaymentSummary" });
            }
            ...
            if (HttpContext.Session.Get<ReservationVM>(SD.ReservationSession) != null)
            {
                ...
                reservationVM = HttpContext.Session.Get<ReservationVM>(SD.ReservationSession);  // Get current reservation view model from saved session

                newReservation = reservationVM.reservationObj;   //storing resevation from resevation view model to new reservation
                paymentObj = reservationVM.paymentObj;   //storing payment form view model to paymentObj
                vehicleType = _unitOfWork.Vehicle_Type.Get(v => v.TypeID == newReservation.TypeID).TypeName;  //getting vehicle type 
        ...
        public async Task<IActionResult> OnPost(string returnUrl = null)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                if (paymentIntent.Status.ToLower() == "succeeded")  //Error as payment.Status is never succeeded
                {
            ...
            return RedirectToPage("/Client/MyReservations", new { error = true });
